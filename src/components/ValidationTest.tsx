...
export const ValidationTest: React.FC = () => {
  ...
  const runValidation = async () => {
    ...
    for (let i = 0; i < validationData.length; i++) {
      ...
      const isValid = result.crispOutput >= testCase.expectedRange[0] && 
  ...
  const accuracy = testResults.length > 0 ? (validCount / testResults.length) * 100 : 0;

  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Validasi Sistem</h2>
        <p className="text-gray-600">
          Uji validitas sistem menggunakan 30 kasus uji yang telah disiapkan untuk memastikan akurasi sistem pakar.
        </p>
      </div>
      ...
      <div className="mb-6">
        ...
        >
          ...
          <span>{isRunning ? `Menguji ${currentTest}/${validationData.length}...` : 'Jalankan Validasi'}</span>
        </button>
      </div>
        ...
        <div className="mb-6">
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div 
              ...
              style={{ width: `${(currentTest / validationData.length) * 100}%` }}
            ></div>
          </div>
        </div>
      )}
      ...
      {testResults.length > 0 && (
        <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-center">
            ...
            <div className="text-2xl font-bold text-red-600">{testResults.length - validCount}</div>
            ...
            <table className="min-w-full table-auto">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Rekomendasi</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {testResults.map((result, index) => (
                  <tr key={index} className={result.isValid ? 'bg-green-50' : 'bg-red-50'}>
                    ...
                    <td className="px-4 py-2 text-sm text-gray-600">{result.description}</td>
                    ...
                    <td className="px-4 py-2 text-sm">{result.recommendation}</td>
